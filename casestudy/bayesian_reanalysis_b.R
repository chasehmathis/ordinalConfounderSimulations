...
full_data1 <- data.frame(cbind(dat1$Y, dat1$T, what1, dat$race, dat$age, 
                               dat$gender, dat$smoking, dat$activity, dat$kcal, 
                               dat$alcohol, dat$bmi,
                               dat$SDMVPSU,
                               dat$SDMVSTRA,
                               dat$WTMEC2YR))
colnames(full_data1) <- c("Y", "T", "what1", "race", "age", 
...
full_data2 <- data.frame(cbind(dat2$Y, dat2$T, what2, dat$race, dat$age, 
                               dat$gender, dat$smoking, dat$activity, dat$kcal, 
                               dat$alcohol, dat$bmi,
                               dat$SDMVPSU,
                               dat$SDMVSTRA,
                               dat$WTMEC2YR))
colnames(full_data2) <- c("Y", "T", "what2", "race", "age", 
                          "gender", "smoking", "activity", "kcal", "alcohol", 
                          "bmi", "SDMVPSU", "SDMVSTRA", "WTMEC2YR")
